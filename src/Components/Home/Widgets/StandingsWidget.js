...
export default function StandingsWidget(props) {
    const [ activeDivision, setActiveDivision ] = useState(props.standings.records[1]);

    return (
        <div>
            <StyledHeader>Standings</StyledHeader>
            <StyledOptionContainer>
                <StyledOption bold={ (activeDivision.division.abbreviation === "A") } onClick = { () => setActiveDivision(props.standings.records[1]) }>Atlantic</StyledOption>
                <StyledOption bold={ (activeDivision.division.abbreviation === "M") } onClick = { () => setActiveDivision(props.standings.records[0]) }>Metropolitan</StyledOption>
                <StyledOption bold={ (activeDivision.division.abbreviation === "P") } onClick = { () => setActiveDivision(props.standings.records[3]) }>Pacific</StyledOption>
                <StyledOption bold={ (activeDivision.division.abbreviation === "C") } onClick = { () => setActiveDivision(props.standings.records[2]) }>Central</StyledOption>
            </StyledOptionContainer>
